...
- `groomer`, `reviewer`, `ledger-scribe`, `docs-auditor`, and `debugger` subagents — turning a roadmap item into a Ready issue, reviewing a diff against its issue's acceptance criteria, appending `docs/ledger.md` rows after a PR merges, sweeping the whole `docs/` tree for Diátaxis/ADR/link/diagram drift, and reproducing/root-causing a live failure, respectively.
