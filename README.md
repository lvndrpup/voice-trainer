...
- `groomer`, `reviewer`, `ledger-scribe`, `docs-auditor`, `accessibility-tester`, and `debugger` subagents — turning a roadmap item into a Ready issue, reviewing a diff against its issue's acceptance criteria, appending `docs/ledger.md` rows after a PR merges, sweeping the whole `docs/` tree for Diátaxis/ADR/link/diagram drift, auditing the canvas-only UI for keyboard/screen-reader/colorblind-contrast access, and reproducing/root-causing a live failure, respectively.
